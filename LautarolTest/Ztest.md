...
10.  <span data-ttu-id="25e66-155">此时，我们需要配置 Xamarin.iOS 项目以使用这个新创建的预配配置文件。</span><span class="sxs-lookup"><span data-stu-id="25e66-155">At this point we need to configure the Xamarin.iOS project to use this newly created provisioning profile.</span></span> <span data-ttu-id="25e66-156">这是从**项目选项**对话框下**iOS 捆绑签名**选项卡，如以下屏幕截图中显示：</span><span class="sxs-lookup"><span data-stu-id="25e66-156">This is done from  **Project Options** dialog, under  **iOS Bundle Signing** tab, as showing in the following screenshot:</span></span>

<span data-ttu-id="25e66-157">[![](remote-notifications-in-ios-images/image11.png "配置 Xamarin.iOS 项目以使用这个新创建的预配配置文件")](remote-notifications-in-ios-images/image11.png#lightbox)</span><span class="sxs-lookup"><span data-stu-id="25e66-157">[![](remote-notifications-in-ios-images/image11.png "Configure the Xamarin.iOS project to use this newly created provisioning profile")](remote-notifications-in-ios-images/image11.png#lightbox)</span></span>

<span data-ttu-id="25e66-158">此时应用程序配置为使用推送通知。</span><span class="sxs-lookup"><span data-stu-id="25e66-158">At this point the application is configured to work with push notifications.</span></span> <span data-ttu-id="25e66-159">但是，仍有几个步骤所需的证书。</span><span class="sxs-lookup"><span data-stu-id="25e66-159">However, there are still a few more steps required with the certificate.</span></span> <span data-ttu-id="25e66-160">此证书是 DER PushSharp，需要的个人信息交换 (PKCS12) 证书与不兼容的格式。</span><span class="sxs-lookup"><span data-stu-id="25e66-160">This certificate is in DER format that is not compatible with PushSharp, which requires a Personal Information Exchange (PKCS12) certificate.</span></span> <span data-ttu-id="25e66-161">若要将证书转换，这样就可供 PushSharp，执行这些最后的步骤：</span><span class="sxs-lookup"><span data-stu-id="25e66-161">To convert the certificate so that it is usable by PushSharp, perform these final steps:</span></span>
...
|||
|-|-|
|<span data-ttu-id="cc05c-144">엔터티 수신기</span><span class="sxs-lookup"><span data-stu-id="cc05c-144">Entity Receiver</span></span>|<span data-ttu-id="cc05c-145">모든 ATA 게이트웨이 및 ATA 경량 게이트웨이에서 엔터티 배치를 받습니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-145">Receives batches of entities from all ATA Gateways and ATA Lightweight Gateways.</span></span>|
|<span data-ttu-id="cc05c-146">네트워크 활동 처리기</span><span class="sxs-lookup"><span data-stu-id="cc05c-146">Network Activity Processor</span></span>|<span data-ttu-id="cc05c-147">받은 각 배치 내의 모든 네트워크 활동을 처리합니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-147">Processes all the network activities within each batch received.</span></span> <span data-ttu-id="cc05c-148">예를 들어 서로 다를 수 있는 컴퓨터에서 수행한 여러 Kerberos 단계가 일치하는지를 확인합니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-148">For example, matching between the various Kerberos steps performed from potentially different computers</span></span>|
|<span data-ttu-id="cc05c-149">엔터티 프로파일러</span><span class="sxs-lookup"><span data-stu-id="cc05c-149">Entity Profiler</span></span>|<span data-ttu-id="cc05c-150">트래픽과 이벤트에 따라 모든 고유 엔터티를 프로파일링합니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-150">Profiles all the Unique Entities according to the traffic and events.</span></span> <span data-ttu-id="cc05c-151">예를 들어 ATA는 각 사용자 프로필에 대해 로그온한 컴퓨터 목록을 업데이트합니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-151">For example, ATA updates the list of logged-on computers for each user profile.</span></span>|
|<span data-ttu-id="cc05c-152">Center 데이터베이스</span><span class="sxs-lookup"><span data-stu-id="cc05c-152">Center Database</span></span>|<span data-ttu-id="cc05c-153">네트워크 활동과 이벤트를 데이터베이스에 기록하는 프로세스를 관리합니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-153">Manages the writing process of the Network Activities and events into the database.</span></span> |
|<span data-ttu-id="cc05c-154">데이터베이스</span><span class="sxs-lookup"><span data-stu-id="cc05c-154">Database</span></span>|<span data-ttu-id="cc05c-155">ATA는 MongoDB를 사용하여 다음과 같은 시스템의 모든 데이터를 저장합니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-155">ATA utilizes MongoDB for purposes of storing all the data in the system:</span></span><br /><br /><span data-ttu-id="cc05c-156">-   네트워크 활동</span><span class="sxs-lookup"><span data-stu-id="cc05c-156">-   Network activities</span></span><br /><span data-ttu-id="cc05c-157">-   이벤트 활동</span><span class="sxs-lookup"><span data-stu-id="cc05c-157">-   Event activities</span></span><br /><span data-ttu-id="cc05c-158">-   고유 엔터티</span><span class="sxs-lookup"><span data-stu-id="cc05c-158">-   Unique entities</span></span><br /><span data-ttu-id="cc05c-159">-   의심스러운 활동</span><span class="sxs-lookup"><span data-stu-id="cc05c-159">-   Suspicious activities</span></span><br /><span data-ttu-id="cc05c-160">-   ATA 구성</span><span class="sxs-lookup"><span data-stu-id="cc05c-160">-   ATA configuration</span></span>|
|<span data-ttu-id="cc05c-161">탐지기</span><span class="sxs-lookup"><span data-stu-id="cc05c-161">Detectors</span></span>|<span data-ttu-id="cc05c-162">탐지기는 기계 학습 알고리즘과 결정적 규칙을 사용하여 네트워크에서 의심스러운 활동과 비정상적인 사용자 동작을 찾습니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-162">The Detectors use machine learning algorithms and deterministic rules to find suspicious activities and abnormal user behavior in your network.</span></span>|
|<span data-ttu-id="cc05c-163">ATA 콘솔</span><span class="sxs-lookup"><span data-stu-id="cc05c-163">ATA Console</span></span>|<span data-ttu-id="cc05c-164">ATA 콘솔은 ATA를 구성하고 네트워크에서 ATA가 검색한 의심스러운 활동을 모니터링하는 데 사용됩니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-164">The ATA Console is for configuring ATA and monitoring suspicious activities detected by ATA on your network.</span></span> <span data-ttu-id="cc05c-165">ATA 콘솔은 ATA 센터 서비스 실행 여부와 관계없이 실행됩니다. 즉, 서비스가 중지되어도 ATA 콘솔은 데이터베이스와 통신할 수 있으면 실행됩니다.</span><span class="sxs-lookup"><span data-stu-id="cc05c-165">The ATA Console is not dependent on the ATA Center service and runs even when the service is stopped, as long as it can communicate with the database.</span></span>|
<span data-ttu-id="cc05c-166">네트워크에서 배포할 ATA 센터의 수를 결정할 때는 다음 조건을 고려하세요.</span><span class="sxs-lookup"><span data-stu-id="cc05c-166">Consider the following criteria when deciding how many ATA Centers to deploy on your network:</span></span>
